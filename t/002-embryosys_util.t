...
test_matrix_element({FromState, [{Operation, ToState}|Rest]}) ->
	...
	etap:is(embryosys_util:new_state_after(Operation, FromState), ToState, Message),
...
test_new_state_after() ->
	...
	etap:diag("Testing embryosys_util:new_state_after."),
	test_matrix(Base).
...
main(_) ->
	...
	%%etap:is( ClassID, embryosys_util:new_id(class), "New ID for class" ),
	%%etap:is( ClassADT, embryosys_util:new_adt(class, ClassName), "New class ADT"),

	%%AttributeID = { adt_id, attribute, { 0, 0 } },
	%%AttributeName = "height",
	%%AttributeADT = { adt, AttributeID, AttributeName, none },
	%%etap:is( AttributeID, embryosys_util:new_id(attribute), "New ID for attribute" ),
	%%etap:is( AttributeADT, embryosys_util:new_adt(attribute, AttributeName), "New attribute ADT"),

	%%LinkID = { adt_id, link, { 0, 0, 0 } },
	%%LinkName = "cross",
	%%LinkADT = { adt, LinkID, LinkName, none },
	%%etap:is( LinkID, embryosys_util:new_id(link), "New ID for link" ),
	%%etap:is( LinkADT, embryosys_util:new_adt(link, LinkName), "New link ADT"),

	%%ObjectID = { adt_id, object, { 0, 0 } },
	%%ObjectName = "Alma",
	%%ObjectADT = { adt, ObjectID, ObjectName, none },
	%%etap:is( ObjectID, embryosys_util:new_id(object), "New ID for object" ),
	%%etap:is( ObjectADT, embryosys_util:new_adt(object, ObjectName), "New object ADT"),
